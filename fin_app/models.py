...
class CompanyTransaction(models.Model):
    ...
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='company_transactions')
    type = models.CharField(max_length=20, choices=TRANSACTION_TYPE)
    amount = models.DecimalField(max_digits=12, decimal_places=2)
    date = models.DateField(default=timezone.now)
    payee_payer = models.CharField(max_length=100)
    month = models.CharField(max_length=20)  # e.g., "July 2025"
    category = models.CharField(max_length=50, blank=True)
    description = models.TextField(blank=True)

    def __str__(self):
        return f"{self.type} - {self.payee_payer} - {self.amount} - {self.month}"