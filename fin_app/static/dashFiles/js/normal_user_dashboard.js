$(document).ready(function () {
    ...
    // Total Categories (Donut Chart)
    const categoryData = chartData.category_data || { labels: [], data: [] };
    console.log(categoryData);
    
    const categoryChart = {
      series: categoryData.data.length ? categoryData.data : [1],
      ...
      colors: ["var(--bs-primary)", "var(--bs-danger)", "var(--bs-success)", "var(--bs-warning)"],
      dataLabels: { enabled: false },
      legend: { show: false },
      stroke: { show: false },
      responsive: [
    ...
    const chart = new ApexCharts(document.querySelector("#grade"), categoryChart);
    chart.render();
  
    // Total Budgeted (Area Chart)
    ...
    const earning = {
      ...
      markers: { size: 0 },
      xaxis: { categories: budgetTrendData.labels.length ? budgetTrendData.labels : ['No Data'] },
      tooltip: {
        theme: "dark",
